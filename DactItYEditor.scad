...
//include <SRC/Layouts/HypoWarp_angled_MX.scad>  
include <SRC/Layouts/HypoWarp_angled_MX_x7.scad>  // (6x3+2)+[trichord+1] Hypothetical Rhetoric.
...
mirror([0,0,0]){ //[100] for Left side
  color(){
    BuildTopPlate(
      ...
      Trackball        = true,  // turn on trackball module. don't its a mess 
      ThumbJoint       = true,   // salmon color turn on thu+mb and column joint module 
      CustomBorder     = true,   // Red manually defined custom border
      ColoredSection   = true    // Color border by module to visualize during edit/debug
    );
  }
