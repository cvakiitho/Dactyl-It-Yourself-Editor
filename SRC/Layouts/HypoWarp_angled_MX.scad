...
RowInits    = [   R2,    R1,   R1,   R0,   R0,   R1,   R1,   R0,   R2,   R2,    R1,   R1,    R0]; //set which Row to begin
RowEnds     = [   R2,    R3,   R3,   R3,   R3,   R3,   R3,   R0,   R2,   R2,    R2,   R1,    R0]; //set which Row to end

//Row transforms
RowTrans    = [[ -.18, -.18, -.18, -.18, -.18, -.55, -.55, -.55, -1.0,-1.25,-1.25,     0,     0], //R0
               [  .80,  .78,  .78,  .78,  .78,  .43,  .43,  .45,    0,    0,  -.1,     0,     0], //R1s
...
//front side
  [[C1, RowEnds[C1], false,FRONT, [0,0,0], [1,RScale,1]], //fill gap between general border and Bottom Enclosure near C1
   [C2, RowEnds[C2], false,FRONT, [0,0,0], [1,RScale,1]]],
   
  [[C2, RowEnds[C2], false,FRONT, [0,0,0],    [1,RScale,1]], //fill gap between general border and Bottom Enclosure near C2
  ...
  [[C2, RowEnds[C2], false,FRONT, [LEFT,0,0],  [1,RScale,1]],
   ...
   [C4, RowEnds[C4], false,FRONT, [RIGHT,0,0], [1,RScale,1]]],
